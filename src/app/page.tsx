...
import { MemoCard } from '@/components/optimized/memo-card';
...
const LeaderboardTable = memo(({ 
  ...
  const getRankComponent = useCallback((rank: number) => {
        ...
        <Table>
          ...
          <TableBody>
            {data.length > 0 ? data.map((entry, index) => (
              <TableRow key={`${entry.agentName}-${index}`} className="hover:bg-accent/50 transition-colors">
                ...
                <TableCell className="text-right font-semibold">
                  {isCurrency ? `$${entry.value.toLocaleString(undefined, {minimumFractionDigits: 2, maximumFractionDigits: 2})}` : entry.value}
                </TableCell>
              </TableRow>
            )) : (
              <TableRow>
...
const TopPerformerCard = memo(({ 
  ...
  return (
    <Card className="relative overflow-hidden bg-gradient-to-br from-primary/10 to-background hover:shadow-xl transition-shadow duration-300">
      <div className="absolute top-0 right-0 p-2 bg-primary/80 text-primary-foreground text-xs font-bold rounded-bl-lg">
        TOP PERFORMER
      </div>
      <CardHeader>
    ...
    <div className="w-full h-full space-y-8">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center">
       ...
       <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
        <TopPerformerCard title="Top Deposits" data={topDeposits} isCurrency={true} icon={<Trophy className="text-yellow-500" />} />
        <TopPerformerCard title="Top Shops Opened" data={topShops} icon={<Store className="text-blue-500" />} />
        <TopPerformerCard title="Top Clients Added" data={topClientsAdded} icon={<UserPlus className="text-green-500" />} />
      </div>

      <div>
        <h2 className="text-3xl font-bold text-foreground mb-4">Leaderboards</h2>
        <div className="grid gap-6 md:grid-cols-1 lg:grid-cols-3">
          <LeaderboardTable title="Top 10 Agent Deposits" data={topDeposits} isCurrency={true} icon={<Trophy className="text-yellow-500" />} iconBgClass="bg-yellow-500/10" />
          <LeaderboardTable title="Top 10 Shops Opened" data={topShops} icon={<Store className="text-blue-500" />} iconBgClass="bg-blue-500/10" />
          <LeaderboardTable title="Top 10 Clients Added" data={topClientsAdded} icon={<UserPlus className="text-green-500" />} iconBgClass="bg-green-500/10" />
        </div>
      </div>
